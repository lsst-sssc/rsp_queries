...
def make_query(q_cutoff=1.3, e_cutoff=1.0, a_cutoff=10.0, a_cutoffmin=5.5):
    if(class_type == "default"):
        ...
        query = f"""a FROM dp03_catalogs_10yr.MPCORB as mpc
            WHERE mpc.a > {a_cutoffmin} AND mpc.a < {a_cutoff};"""

    elif(class_type == "mbas"):
        query = f"""a, q FROM dp03_catalogs_10yr.MPCORB as mpc
        ...
        query = f"""a, e FROM dp03_catalogs_10yr.MPCORB as mpc
